...
import {
  DownloadFlaggedCaseByIdRequest,
...
const FlaggedInformation = ({ setSearchFilter, searchFilter }) => {
    ...
    columnHelper.accessor("details", {
      cell: (info) => (
        <span className="font-normal">
          ...
          {info?.row?.original?.caseDetails?.nameParties}{" "}
          ...
          {showActions && row?.original?._id === selectedRow?._id && (
            <div className="bg-white shadow-md rounded-[.25rem] border border-[#213f7d0f] space-y-3 absolute z-[1] top-[32px] left-[-80px] w-[120px]">
              <div ref={viewRef}>
                ...
                <p
                  className="p-3 gap-2  flex cursor-pointer hover:bg-gray-100 border-[1.2px] border-slate-200"
                  onClick={handledDownloadFlaggedCase}
                >
  ...
  // Download Flagged Case by case Id Logic
  const handledDownloadFlaggedCase = async () => {
    try {
      await DownloadFlaggedCaseByIdRequest(token, selectedRow?._id);
      toast.success("Download Case Successful");
      queryClient.invalidateQueries("getFlaggedCaseApi");
    } catch (error) {
      console.log(error?.response?.data?.message);
      toast.error(error?.response?.data?.message);
    ...
    <>
      ...
      >
        <FlaggedDetails
          ...
          handledDownloadFlaggedCase={handledDownloadFlaggedCase}
        />
      </Modal>
      <ToastContainer />
    </>
