...
import LogDetails from "../activities-logs/LogDetails";
...
const Recent = ({
    ...
    columnHelper.accessor("avatar", {
      cell: (info) => (
        <div className="w-[20px] lg:w-fit">
          {info?.row?.original?.userId?.email ? (
  ...
  const handleDelete = async () => {
    try {
      ...
      queryClient.invalidateQueries("getDashboardActivityLogsApi");
    } catch (error) {
  ...
  return (
    <>
      <div>
        <section className="bg-white mt-8 rounded-2xl p-8">
          {activityLogsData?.data?.logs?.length > 0 && (
            <div className="md:flex space-y-4 md:space-y-0 justify-between">
              ...
              <Link
                to={
                  activityLogsData?.data?.logs?.length >= 3
                    ? "more-recent"
                    : "#"
                }
          ...
          <div className="w-full  rounded-lg  overflow-hidden">
            <div className="h-[220px] rounded-tl-[8px] rounded-tr-[8px] bg-white">
              {/* Table */}
              {isLoading ? (
