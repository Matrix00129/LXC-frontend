...
const UploadedCases = ({ searchFilter }) => {
  ...
  const columns = [
    ...
    columnHelper.accessor("propertyLocation", {
      cell: (info) => (
        <span className="">{info?.row?.original?.propertyLocation}</span>
      ),
      ...
      <div>
        <section>
          <div className="mt-10 bg-white rounded-2xl p-4 md:p-8">
            ...
            {/* TABLE SECTION */}
            <div className="w-full  border-[1.3px] border-[#E8E8E8] p-6 rounded-2xl  overflow-hidden mt-6">
              <div className="h-[240px] rounded-tl-[8px] rounded-tr-[8px] bg-white">
                {/* Table */}
                {isLoading ? (
